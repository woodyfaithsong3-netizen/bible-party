...
export default function CustomizeScreen() {
  ...
  return <ScenicScreen>
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[styles.flowContent, { paddingBottom: 150 }]}> 
      ...
      <View style={{ flexDirection: 'row', gap: 8 }}>{difficultyOptions.map(([id, label]) => <Pressable key={id} onPress={() => setDifficulty(id)} style={[styles.smallChoice, difficulty === id && styles.smallChoiceActive]}><Text style={[styles.smallChoiceText, difficulty === id && { color: colors.bg }]}>{label}</Text></Pressable>)}</View>
