...
export default function GameScreen() {
  ...
  const [timer, setTimer] = useState(10); const [running, setRunning] = useState(false); const [paused, setPaused] = useState(false); const [confirmQuit, setConfirmQuit] = useState(false);
  const [roundIntro, setRoundIntro] = useState(true);
  const [gameLeft, setGameLeft] = useState(duration * 60); const deadline = useRef(Date.now() + duration * 60000); const roundDeadline = useRef<number | null>(null);
  ...
  const reset = useCallback(() => {
    setSelected(null); setTf(null); setClue(0); setShowMaster(false); setChronology([]); setIntruder(null); setRevealed(false); setValidated(false); setCorrect(null); setDelta(0);
    const seconds = mode === 'timesup' ? 30 : mode === 'challenge' ? (question.type === 'challenge' ? question.seconds : 10) : 0;
    setTimer(seconds); setRunning(false); roundDeadline.current = null; setRoundIntro(true);
  }, [mode, question]);
  useEffect(() => { reset(); }, [round, mode, question.id, reset]);
  useEffect(() => { getSettings().then(setSettings); }, []);
  useEffect(() => { if (paused || ended.current) return; const id = setInterval(() => { const left = Math.max(0, Math.ceil((deadline.current - Date.now()) / 1000)); setGameLeft(left); if (!left && !ended.current) { ended.current = true; finish(); } }, 250); return () => clearInterval(id); }, [paused]);
  ...
  function startTimed() {
    if (validated || paused) return;
    if (running) { setRunning(false); roundDeadline.current = null; setRevealed(true); return; }
  ...
  const chosenReady = mode === 'truefalse' ? tf !== null : mode === 'chronology' ? chronology.length === chronologyItems.length : mode === 'intruder' ? intruder !== null : selected !== null;
